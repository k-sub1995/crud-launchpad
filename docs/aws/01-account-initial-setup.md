...
rootユーザーはアカウントに対して制限のない権限を持ちます。  
パスワードだけで守られている状態は非常に危険です（流出すると乗っ取られます）。  
MFAを設定するとパスワードに加えてスマホの認証アプリ等が生成する使い捨てコードも必要になり、乗っ取られにくくなります。
...
今回はハンズオンで様々なサービスを横断的に触るため、簡易的に`AdministratorAccess`を使います。  
実務でこの権限を使う場合は、rootユーザーと同様に「持っているだけでリスクが高い」ことを意識し、常用アカウントには必要な操作だけを許可するポリシーを個別に設定します。
...
パスワードのリセットを行います。  
（.csvに書かれたパスワードは作成者[root]も知っている一時的なものです。自分だけが知っているパスワードに変更するため、初回サインイン時にリセットが求められます。）  
