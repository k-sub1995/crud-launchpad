...
### 3. バケットポリシーをCloudFront用に更新し、ブロックパブリックアクセスを有効に戻す

現状では、03資料の手順10で設定した公開読み取りポリシーが残っているため、S3から直接アクセスできてしまいます。  
CloudFront(OAC)経由のみアクセスにするため、バケットポリシーを更新してから、S3の直接公開を閉じます。
