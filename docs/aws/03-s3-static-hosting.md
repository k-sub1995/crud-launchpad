...
![S3単体の静的ウェブサイトホスティング構成図](s3-static-hosting-architecture.svg)

この構成図では、AWS公式アーキテクチャアイコンで、ユーザーアクセス、S3ウェブサイトエンドポイント、公開S3バケットの関係を示します。
...
4. ブロックパブリックアクセスのチェックを外します。  
   ...
   ※このチェックを外しただけではまだ何も公開されません。  
   「公開してよい」という制限解除にすぎず、実際の公開は手順10の後です。
5. バケットを作成します。  
   ※途中に `デフォルトの暗号化` という設定がありますが、これは保存したファイルをAWS側で自動暗号化する設定です。  
   デフォルトの `SSE-S3` のまま進めます。  
